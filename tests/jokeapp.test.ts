...
// TEST FOR EMPPTY FILES CSV.
it('should return an empty array when parsing an empty CSV stream', async () => {
    const parser = new JokeApp();
    const emptyCsvData = 'id,joke,punchline\n';
    const readableStream = Readable.from([emptyCsvData]);   
    const jokes = await parser.parseCsvStream(readableStream);
...
// TEST FOR EMPPTY FILES JSON.
it('should return an empty array when parsing an empty JSON stream', async () => {
    const parser = new JokeApp();
    const emptyJsonData = JSON.stringify([]);
    const readableStream = Readable.from([emptyJsonData]);
    const jokes = await parser.parseJsonStream(readableStream);
